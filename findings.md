...
| Desktop roadmap / lesson previews were always empty after relaunch | `apps/desktop/renderer/src/App.tsx` only kept generated plan, lesson, history, and replan in React memory; added a main-process JSON store under Electron `userData`, IPC load/save methods, startup hydration, and schema-validated restore so the last desktop state survives restarts |
