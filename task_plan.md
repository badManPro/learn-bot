...
- The desktop auth-to-model gap is now closed in code: Electron main reuses refreshed Learn Bot / Codex OAuth access tokens for model calls, and `OPENAI_API_KEY` is only a development fallback when no local desktop session exists.
- The desktop auth direction has now been corrected again: the custom OpenAI OAuth experiment was a dead end, and the supported desktop path is to mirror JARVIS by using official `codex login` for auth plus `codex exec` for model access.
...
- 2026-04-14: Desktop roadmap / lesson state also needs explicit persistence; the renderer currently only keeps previews in memory, so restart-safe behavior requires a main-process store plus startup hydration.
